...
class orders extends cdek_integrator implements exchange {
	...
	private function createXML($data = array()) {
		...
		if (!empty($data)) {
			...
			$xml .= '<DeliveryRequest Number="' . $this->number . '" Date="' . $this->date . '" IP ="' . $this->getIp() . '" Account="' . $this->account . '" Secure="' . $this->getSecure() . '" OrderCount="' . count($data) . '" DeveloperKey="c6e7304995e8e1f513f1ec380ff89779">';
			...
			foreach ($data as $order_info) {
				...
				if (in_array($tariff_info['mode_id'], array(2, 4, 6))) {					
